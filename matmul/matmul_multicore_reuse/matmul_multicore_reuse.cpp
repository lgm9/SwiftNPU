...
void matmul_multicore_reuse(
    ...
    uint32_t num_cores_x = compute_with_storage_grid_size.x;  // 13
    uint32_t num_cores_y = compute_with_storage_grid_size.y;  // 10
    fmt::print(
    ...
    uint32_t per_core_M = std::get<0>(matmul_params);      // 20
    uint32_t per_core_N = std::get<1>(matmul_params);      // 2
    uint32_t out_subblock_h = std::get<2>(matmul_params);  // 4
    uint32_t out_subblock_w = std::get<3>(matmul_params);  // 2
    ...
    for (int output_idx_y = 0; output_idx_y < num_blocks_y; output_idx_y++) {
        for (int output_idx_x = 0; output_idx_x < num_blocks_x; output_idx_x++) {
            ...
            fmt::print("in0_tensor_start_tile_id: {}\n", (std::uint32_t)Kt * per_core_M * output_idx_y);
            fmt::print("in1_tensor_start_tile_id: {}\n", (std::uint32_t)per_core_N * output_idx_x);
            fmt::print("num_blocks: {}\n", (std::uint32_t)Kt / in0_block_w);
...
int main(int argc, char* argv[]) {
    ...
    options.add_options()("core_x", "Core X coordinate", cxxopts::value<int>()->default_value("0"))(
        "core_y", "Core Y coordinate", cxxopts::value<int>()->default_value("0"))(
        "iters", "Number of iterations", cxxopts::value<uint32_t>()->default_value("100000"))(
        "flush", "Flush flag", cxxopts::value<uint32_t>()->default_value("1000"))(
        "num_tiles", "Number of Tiles", cxxopts::value<uint32_t>()->default_value("640"))(
        "page_tiles", "Tiles per DRAM page (interleave granularity)", cxxopts::value<uint32_t>()->default_value("1"))(
        "out_dir", "Output DIR", cxxopts::value<std::string>()->default_value("results"))(
        "out", "Output file", cxxopts::value<std::string>()->default_value("result_loopback.txt"));

    cxxopts::ParseResult parse_result = options.parse(argc, argv);
    const uint32_t NUM_TILES = parse_result["num_tiles"].as<uint32_t>();  // Ensure this is used to avoid unused warning
    ...
    try {
        ...
        /* Create source data */
        const uint32_t M = NUM_TILES;  // user-defined
        const uint32_t N = NUM_TILES;  // user-defined
        const uint32_t K = NUM_TILES;  // user-defined
        ...
        std::cout << "golden_matmul finished computing " << "\n";
        ...
        matmul_multicore_reuse(src0_vec, src1_vec, result_vec, false, M, N, K, B, device, parse_result);
