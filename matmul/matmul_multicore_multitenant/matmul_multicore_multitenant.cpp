...
// The number of tenants to run on Tenstorrent BH is not made into options yet.
...
void multi_core_for_one_tenant(
    ...
    std::cout << "Number of cores requested: " << core_range_set.num_cores() << "\n" << std::flush;
    std::cout << "Total number of output tiles: " << num_output_tiles_total << "\n" << std::flush;

    auto [num_cores, all_cores, core_group_1, core_group_2, work_per_core1, work_per_core2] =
        split_work_to_cores(core_range_set, num_output_tiles_total);
    std::cout << "all_cores: " << all_cores.str() << "\n" << std::flush;
    std::cout << "core_group_1: " << core_group_1.str() << "\n" << std::flush;
    std::cout << "core_group_2: " << core_group_2.str() << "\n" << std::flush;
...
void matmul_multi_core(
    ...
    // Here, you need to manually specify the core ranges for each tenant.
    std::vector<CoreRange> core_ranges = {
        CoreRange(CoreCoord{0, 0}, CoreCoord{5, 4}),
        CoreRange(CoreCoord{6, 0}, CoreCoord{11, 4}),
        CoreRange(CoreCoord{0, 5}, CoreCoord{5, 9}),
        // CoreRange(CoreCoord{6, 5}, CoreCoord{11, 9})
    };
    ...
    for (int i = 0; i < NUM_TENANTS; i++) {
        ...
        multi_core_for_one_tenant(
            cq, program, src0, src1, dst, core_ranges[i], Mt, Nt, Kt, single_tile_size, num_output_tiles_total);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        matmul_multi_core(src0_vec, src1_vec, result_vec, M, N, K, device, outfile);
