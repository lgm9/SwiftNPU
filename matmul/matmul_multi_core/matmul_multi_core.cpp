...
void matmul_multi_core(
    ...
    std::cout << "Total number of cores: " << i << "\n";
    ...
    // measure time taken for moving data to DRAM buffers
    EnqueueWriteBuffer(cq, src0_dram_buffer, a.data(), true);
    Finish(cq);
    t1 = __rdtscp(&aux);
    std::cout << "Time taken 1: " << (t1 - t0) << "\n";
    t0 = t1;
    EnqueueWriteBuffer(cq, src1_dram_buffer, b.data(), true);
    Finish(cq);
    t1 = __rdtscp(&aux);
    std::cout << "Time taken 2: " << (t1 - t0) << "\n";
    t0 = t1;

    // measure time taken for matrix multiplication
    for (int i = 0; i < 10; i++) {  // ----- Timing + batched execution -----
        t0 = __rdtscp(&aux);
    ...
    // measure time taken for moving data from DRAM buffers to output vector
    EnqueueReadBuffer(cq, dst_dram_buffer, output.data(), true);
    Finish(cq);
    t1 = __rdtscp(&aux);
    std::cout << "Time taken last: " << (t1 - t0) << "\n";
    t0 = t1;
}
